...
export default function Portfolio() {
    ...
    useEffect(() => {
        if (selectTab === 'all') {
            ...
            const filteredData = data.filter((item) => 
                item.category.toLowerCase() === selectTab.toLowerCase()
            );
            setPortfolios(filteredData);
        }
    }, [selectTab]);

    // Get the active portfolio item based on activeId
    ...
    return (
        <section id="portfolio">
            <div className="container">
                ...
                <div className="flex items-center gap-4 flex-wrap">
                    {portfolios.slice(0, nextItems)?.map((portfolio, index) => (
                        ...
                        >
                            <figure>
                                <img className="rounded-[8px]" src={portfolio.imgUrl} alt={`Project ${index + 1}`} />
                            </figure>
                ...
                <div className="text-center mt-6">
                    {nextItems < portfolios.length && data.length > 6 && (
                        <button
                            ...
                            className="text-white bg-headerColor hover:bg-smallTextColor py-2 px-4 rounded-[8px] font-[500] ease-in duration-200"
                        >
                            Load More
                        </button>
                    )}
            ...
            {/* Render the Modal with the active portfolio data */}
            {showModal && activePortfolio && (
                <Modal
                    activeId={activeId}
